 ...
 * @return array
 */
function simplebit_body_classes( $classes ) {
...
add_filter( 'body_class', 'simplebit_body_classes' );

/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function simplebit_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'simplebit_pingback_header' );
 ...
 * @return array
 */
function simplebit_filter_script_loader_tag( $tag, $handle ) {
...
add_filter( 'script_loader_tag', 'simplebit_filter_script_loader_tag', 10, 2 );
 ...
 */
function simplebit_get_preload_stylesheet_uri( $wp_styles, $handle ) {
 ...
 */
function simplebit_add_body_style() {
	...
	// Preload content.css.
	$preloads['simplebit-content'] = simplebit_get_preload_stylesheet_uri( $wp_styles, 'simplebit-content' );

	// Preload sidebar.css and widget.css.
	if ( is_active_sidebar( 'sidebar-1' ) ) {
		$preloads['simplebit-sidebar'] = simplebit_get_preload_stylesheet_uri( $wp_styles, 'simplebit-sidebar' );
		$preloads['simplebit-widgets'] = simplebit_get_preload_stylesheet_uri( $wp_styles, 'simplebit-widgets' );
	}

	// Preload comments.css.
	if ( ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() ) ) {
		$preloads['simplebit-comments'] = simplebit_get_preload_stylesheet_uri( $wp_styles, 'simplebit-comments' );
	}

	// Preload front-page.css.
	global $template;
	if ( 'front-page.php' === basename( $template ) ) {
		$preloads['simplebit-front-page'] = simplebit_get_preload_stylesheet_uri( $wp_styles, 'simplebit-front-page' );
	}
...
add_action( 'wp_head', 'simplebit_add_body_style' );
 ...
 */
function simplebit_add_primary_menu_dropdown_symbol( $item_output, $item, $depth, $args ) {
...
add_filter( 'walker_nav_menu_start_el', 'simplebit_add_primary_menu_dropdown_symbol', 10, 4 );
 ...
 */
function simplebit_add_nav_menu_aria_current( $atts, $item ) {
	/*
...
add_filter( 'nav_menu_link_attributes', 'simplebit_add_nav_menu_aria_current', 10, 2 );
add_filter( 'page_menu_link_attributes', 'simplebit_add_nav_menu_aria_current', 10, 2 );
